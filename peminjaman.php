...
<div class="container-fluid">
    <div class="row">
        ...
        <div class="col-md-10 p-4">
            ...
            <!-- ================= TABEL 0: MENUNGGU PERSETUJUAN (GROUPING) ================= -->
                    ...
                    <table class="table table-hover table-bordered mb-0">
                        ...
                        <tbody>
                            ...
                            $sql_acc = "SELECT nama_mahasiswa, COUNT(*) as total 
                                        ...
                                        GROUP BY nama_mahasiswa, id ORDER BY id ASC";
                    ...
                    <table class="table table-hover table-bordered mb-0">
                        ...
                        <tbody>
                            ...
                            $sql_aktif = "SELECT nama_mahasiswa, COUNT(*) as total 
                                          ...
                                          GROUP BY nama_mahasiswa, id ORDER BY id DESC";
...
function showDetail(data, nama, tipe) {
    ...
    data.forEach(item => {
        ...
        if (tipe === 'pending') {
            // Jika tipe Pending: Tampilkan tombol Setujui dan Tolak
            tombolAksi = `
                <div class="btn-group">
                    <a href="proses_pinjam.php?setujui=${item.id}" class="btn btn-sm btn-success py-0" onclick="return confirm('Setujui?')">Setujui</a>
                    <a href="proses_pinjam.php?tolak=${item.id}" class="btn btn-sm btn-outline-danger py-0" onclick="return confirm('Tolak?')">Tolak</a>
                </div>`;
        } else if (tipe === 'aktif') {
            // Jika tipe Aktif: Tampilkan tombol Kembalikan
            tombolAksi = `
                <a href="proses_pinjam.php?kembalikan=${item.id}" class="btn btn-sm btn-danger py-0" onclick="return confirm('Kembalikan?')">
                    <i class="bi bi-arrow-return-left"></i> Kembalikan
                </a>`;
        } else {
            // Jika tipe Selesai: Tampilkan Badge Selesai
