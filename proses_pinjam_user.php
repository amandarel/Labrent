...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['id_alat'])) {
    ...
    try {
        ...
        foreach ($ids_alat as $index => $id_alat) {
            ...
            // 4. Potong Stok (Stok dipotong saat mengajukan agar tidak ditikung mahasiswa lain)
            $stmt_upd = $pdo->prepare("UPDATE alat SET jumlah_alat = jumlah_alat - ? WHERE id = ?");
            $stmt_upd->execute([$jml_minta, $id_alat]);
        }

        $pdo->commit();
        echo "<script>alert('Berhasil! Pengajuan pinjaman sedang menunggu persetujuan admin.'); window.location='pinjam.php';</script>";

    } catch (Exception $e) {
        $pdo->rollBack();
        echo "<script>alert('Gagal: " . $e->getMessage() . "'); window.location='pinjam.php';</script>";
    }
} else {
